...
class TransitionDecisionTool:
    ...
    async def execute(self,
        ...
        if result == TransitionDecision.END_CONVERSATION and unexplored_types:
            ...
            reasoning = TransitionReasoning(
                reasoning=f"Invalid END_CONVERSATION decision - unexplored_types not empty: "
                          f"{[wt.name for wt in unexplored_types]}. "
                          f"Original reasoning: {reasoning.reasoning if reasoning else 'None'}",
                confidence="high"
            )
    ...
    async def _internal_execute(self,
                                *,
                                ...
                                prompt: str,
                                unexplored_types: list[WorkType],
                                ) -> tuple[TransitionDecision, TransitionReasoning, list[LLMStats], float, BaseException | None]:
...
An experience is incomplete if it has a title but missing start_date, end_date, or company.
...
def _find_incomplete_required_for_work_type(
    ...
    Optional fields (start_date, end_date, company) are NOT checked here.
    """
