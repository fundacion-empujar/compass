...
const LOGO_CONFIG = [
  ...
  { imageUrl: ReportContent.IMAGE_URLS.CIRION_LOGO, width: 55, height: 20 },
  ...
  { imageUrl: ReportContent.IMAGE_URLS.MERCADOLIBRE_LOGO, width: 78, height: 20 },
...
const HeaderComponent = async () => {
  ...
  const logoTable = new Table({
    rows: [
      new TableRow({ children: imageRuns.slice(0, 5).map(makeCell) }),
      new TableRow({ children: imageRuns.slice(5, 10).map(makeCell) }),
    ],
